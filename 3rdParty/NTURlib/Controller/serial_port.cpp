...
const int COUNT_RATIO[] = {1111,1422,1138,1422,1138,-1707,7169,0,0,0,0,0,0,0,0,0};  //  黑金剛Encoder Count ratio
...
//  FAULHABER 自己加的
void SerialPort::FAULHABER_Initial(char* COMport, int m_baudrate)
{
	bool check = false;
	//check = Sport.open("\\\\.\\COM13",NORMAL_RS232);	//開port
	check = open( COMport, NORMAL_RS232);	//開port
	check = _set_baudrate(m_baudrate);	
...
void SerialPort::FAULHABER_Write_and_Read(int command[], int encoder[])
{

}

void SerialPort::FAULHABER_WriteCommand(float Comm[], int type)
{
	char str[70];
	std::string FAULHABER_command;

	int command[6];

	for(int j = 0 ; j < 6 ; j++)
	{
		command[j] = (int)(Comm[j]*COUNT_RATIO[j]);
		//printf("command[%d] = %d\n",j,command[j]);
	}
	switch (type) {
	case 1:
		sprintf(str,"1la%d\rm\r",command[0]);
		break;
	case 2:
		sprintf(str,"2la%d\rm\r",command[1]);
		break;
	case 3:
		sprintf(str,"3la%d\rm\r",command[2]);
		break;
	case 4:
		sprintf(str,"4la%d\rm\r",command[3]);
		break;
	case 5:
		sprintf(str,"5la%d\rm\r",command[4]);
		break;
	case 6:
		sprintf(str,"6la%d\rm\r",command[5]);
		break;
	default:
		sprintf(str,"1la%d\r2la%d\r3la%d\r4la%d\r5la%d\r6la%d\rm\r",command[0],command[1],command[2],command[3],command[4],command[5]);
	}

	FAULHABER_command = str;
	_write((uchar* )FAULHABER_command.c_str(),FAULHABER_command.length());

	//printf("%d / %d\n",write_command.length(),suc3);
}